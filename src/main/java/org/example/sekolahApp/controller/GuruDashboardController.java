...
public class GuruDashboardController implements Initializable {
    ...
    private void loadJadwalMengajar(int staffId) {
        ...
        String sql = "SELECT k.nama_kelas, j.hari, j.jam_mulai, j.jam_selesai, " +
                ...
                "CASE j.hari WHEN 'Senin' THEN 1 WHEN 'Selasa' THEN 2 WHEN 'Rabu' THEN 3 WHEN 'Kamis' THEN 4 WHEN 'Jumat' THEN 5 WHEN 'Sabtu' THEN 6 ELSE 7 END, j.jam_mulai";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, staffId);
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                jadwalList.add(new JadwalGuruView(rs.getString("nama_kelas"), rs.getString("hari"),
                        rs.getTime("jam_mulai").toLocalTime().toString(),
                        rs.getTime("jam_selesai").toLocalTime().toString(),
                        rs.getString("nama_mapel"), rs.getString("tahun_ajaran")));
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    // --- METODE BARU DITAMBAHKAN DI SINI UNTUK MEMPERBAIKI ERROR ---
    @FXML
    private void handleRefresh() {
        int staffId = UserSession.getInstance().getReferenceId();
        // Panggil kembali metode untuk memuat ulang data dari database
        loadBiodataGuru(staffId);
        loadJadwalMengajar(staffId);
        showAlert(Alert.AlertType.INFORMATION, "Refresh", "Data telah berhasil diperbarui.");
    }
    ...
    private void handleLogout() {
        try {
            UserSession.cleanUserSession(); // Gunakan metode statis
